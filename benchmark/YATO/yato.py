...
from main import *
import re
from utils.tools import  save_params
...
class YATO:
    def __init__(self, config,config_dict=None):
        ...
        self.set_config_from_dic(config_dict)
        self.keep_reproducible()
        self.config_dict = config_dict

    def set_config_from_dic(self,config_dict):
        if config_dict:
            data_dict = self.data.__dict__
            # print(data_dict)
            data_dict.update(config_dict)
            check_dir(data_dict['model_dir'])
            save_params(data_dict)
            objDictTool.to_obj(self.data, **data_dict)
    ...
    def keep_reproducible(self):
        ...
        # zeqiang
            ...
            print('You need to set random seed to make the results reproducible!')
    ...
    def predict(self, input_text=None, predict_file=None, write_decode_file=True):
        ...
        self.data.read_config(self.config)
        self.set_config_from_dic(self.config_dict)
        print("MODEL: Predict")
